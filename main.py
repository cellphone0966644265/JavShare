...
def step_join_files(file_list, output_name):
    ...
    print(f" -> Nối file thất bại: {result.get('message')}")
    return []
...
def step_split_files(file_list, max_gb, split_times):
    ...
    if split_times:
        ...
        cmd = [
            ...
            '--start-times'
        ] + [str(t) for t in split_times]
        
        result = run_module(cmd)
        if result.get('status') == 'manual_split':
            final_files.extend(result.get('files', []))
        else:
            print(f" -> Chia file thủ công thất bại cho: {os.path.basename(file_to_split)}.")
        
        # Thêm các file còn lại không được xử lý vào danh sách cuối cùng
        final_files.extend(file_list[1:])

    # Nếu không, chạy chế độ tự động
        ...
        for f in file_list:
            ...
            result = run_module(cmd)
            if result.get('status') in ['auto_split', 'unsplit']:
                final_files.extend(result.get('files', []))
            else:
                print(f" -> Chia file tự động thất bại cho {os.path.basename(f)}. Bỏ qua file này.")
    # Nếu không có tùy chọn chia nào, trả về danh sách file ban đầu
    else:
        return file_list
...
def uploader_task(host, file_path, creds, excel_config):
    ...
    if not os.path.exists(uploader_script):
        print(f"Lỗi: Không tìm thấy script uploader: {uploader_script}"); return
...
def step_upload_files(file_list, upload_hosts, excel_config):
    ...
    for host in upload_hosts:
        ...
        if not creds:
            print(f"Cảnh báo: Không có tài khoản cho '{host}' trong accounts.ini. Bỏ qua."); continue
        for f in file_list:
...
def find_torrent_file(torrent_dir, task_name):
    """Tìm file .torrent trong thư mục một cách linh hoạt."""
...
def workflow_download(args, config):
    ...
    for i, task in enumerate(pending_tasks):
        ...
        if args.workflow == 'url-download':
            link = task.get('Url')
            if link: cmd_download.extend(['-u', link, '-n', task_name])
        elif args.workflow == 'magnet-download':
            link = task.get('MagnetLink')
            if link: cmd_download.extend(['-m', link])
        elif args.workflow == 'torrent-download':
            link = find_torrent_file(config['torrent_dir'], task_name)
            if link: cmd_download.extend(['-t', link])
        
        if not link: print(f"Lỗi: Tác vụ '{task_name}' thiếu link. Bỏ qua."); continue
        
        download_result = run_module(cmd_download)
        if download_result.get('status') != 'success': print(f" -> Tải về thất bại: {download_result.get('message')}"); continue
        ...
        # 2. Đổi tên file
        renamed_files = step_rename_files(newly_downloaded_files, task_name)
...
def workflow_process_local(args, config):
    ...
    renamed_files = step_rename_files(initial_files, args.output_name)
